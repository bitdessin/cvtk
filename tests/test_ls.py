...
class TestDemoAppDeployment(unittest.TestCase):
    ...
    def _train_and_deploy_model(self, backend, task, vanilla, code_generator='api'):
        ...
        if code_generator == 'api':
            ...
            cmd = ['cvtk', 'deploy-ls-mlbackend', app_name, script_path, weights_path, label_path]
            testutils.run_cmd(cmd)
    ...
    # Detection with PyTorch (Faster R-CNN)
    ...
    # Instance Segmentation with PyTorch (Mask R-CNN)
    ...
    # Instance Segmentation with MMDetection (Mask R-CNN)
