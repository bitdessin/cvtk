...
def train(
    ...
    train = cvtk.ml.mmdetapi.DataLoader(
                cvtk.ml.mmdetapi.Dataset(datalabel, train,
                        cvtk.ml.mmdetapi.DataPipeline(is_train=True, with_bbox=True, with_mask=True)),
                phase='train',
    ...
    if valid is not None:
        valid = cvtk.ml.mmdetapi.DataLoader(
                    cvtk.ml.mmdetapi.Dataset(datalabel, valid,
                            cvtk.ml.mmdetapi.DataPipeline(is_train=False, with_bbox=True, with_mask=True)),
                    phase='valid',
                    batch_size=batch_size,
                    num_workers=num_workers)
    if test is not None:
...
def inference(label, data, model_weights, output, batch_size=4, num_workers=8):
    ...
    model = cvtk.ml.mmdetapi.SegmRunner(datalabel, os.path.splitext(model_weights)[0] + '.py', model_weights, workspace=output)
